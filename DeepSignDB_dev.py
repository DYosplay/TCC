...
def centroidNormSize(crt, coords=[0, 1]):
	...
	# m = numpy.min(pps, axis=0)
	# M = numpy.max(pps, axis=0)
	# pps = (pps - numpy.mean(pps, axis=0)) / numpy.max(M - m) 
...
def padAvergeSpeed(crt, indicator, coords=[0, 1]):
	...
	for i in range(0,len(rowIdx)-1):
		...
		segment = crt[inds[rowIdx[i]]:inds[rowIdx[i]]+segLen, [0, 1]]
...
finger = False
if finger:
	path = ".."+ os.sep + "Data"+ os.sep + "DeepSignDB"+ os.sep + "Development"+ os.sep + "finger"
else:
	path = ".."+ os.sep + "Data"+ os.sep + "DeepSignDB"+ os.sep + "Development"+ os.sep + "stylus"
usersMCYT = {}
usersBSID = {}
usersBSDB2 = {}
usersEBio1 = {}
...
for p, dirs, files in os.walk(path):
	...
	for fidx, fn in enumerate(fs):
		...
		### padAvergeSpeed is used to replace real pen-ups with virtual pen-ups. Used for stroke-based synthesis.
...
if finger:
	# u0387_g_u150_s2_g1_b1_sign_w3_c_008, line number is 655, but actually is 321
	if len(usersEBio1) > 0:			
		output = open('..' + os.sep + 'data' + os.sep + 'EBio1_dev_finger.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:			
		output = open('..' + os.sep + 'data' + os.sep + 'EBio2_dev_finger.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()
elif virtualPenup:
	if len(usersMCYT) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'MCYT_dev_pad.pkl', 'wb')
		pickle.dump(usersMCYT, output)
		output.close()

	if len(usersBSID) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'BSID_dev_pad.pkl', 'wb')
		pickle.dump(usersBSID, output)
		output.close()

	if len(usersBSDB2) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'BSDB2_dev_pad.pkl', 'wb')
		pickle.dump(usersBSDB2, output)
		output.close()
				
	if len(usersEBio1) > 0:			
		output = open('..' + os.sep + 'data' + os.sep + 'EBio1_dev_pad.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'EBio2_dev_pad.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()
else:
	if len(usersMCYT) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'MCYT_dev.pkl', 'wb')
		pickle.dump(usersMCYT, output)
		output.close()

	if len(usersBSID) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'BSID_dev.pkl', 'wb')
		pickle.dump(usersBSID, output)
		output.close()

	if len(usersBSDB2) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'BSDB2_dev.pkl', 'wb')
		pickle.dump(usersBSDB2, output)
		output.close()
				
	if len(usersEBio1) > 0:			
		output = open('..' + os.sep + 'data' + os.sep + 'EBio1_dev.pkl', 'wb')
		pickle.dump(usersEBio1, output)
		output.close()

	if len(usersEBio2) > 0:
		output = open('..' + os.sep + 'data' + os.sep + 'EBio2_dev.pkl', 'wb')
		pickle.dump(usersEBio2, output)
		output.close()