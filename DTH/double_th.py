...
def new_evaluate(comparison_file : str, n_epoch : int, result_folder : str, dists_s : Dict[str, float], dists_r : Dict[str, float]):
    ...
    distances_s = distances_s[distances_s <= 0.93]
    distances_s = distances_s[distances_s >= 0.83]
    distances_r = np.array(list(sorted(dists_r.values())))
    distances_r = distances_r[distances_r <= 0.55]
    distances_r = distances_r[distances_r >= 0.45]
