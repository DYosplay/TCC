...
from Losses import triplet_loss, triplet_mmd, compact_triplet_mmd, triple_triplet_loss, compact_quadruplet_mmd, compact_triplet_mmd_entropy, compact_triplet_mmd_alt, clustering_triplet_mmd, contrastive_loss, triplet_loss_offset, compact_triplet_mmd_offset, clustering_triplet_loss, hard_triplet_loss_avg, hard_triplet_loss_max, triplet_distillation, tune_loss, compact_triplet_mmd2, compact_triplet_mmd_scr, compact_triplet_mmd_slice, window_loss, window_loss2,compact_triplet_mmd_random, compact_triplet_mmdx, compact_triplet_mmd_gaussian, double_anchor_ctm, syn_compact_triplet_mmd, compact_triplet_mmd_adapt
from typing import List, Tuple, Dict, Any
import torch
import json
import os

def define_loss(loss_type : str, ng : int, nf : int, nw : int, margin : float, random_margin : float, model_lambda : float, alpha : torch.nn.Parameter, beta : torch.nn.Parameter, p : torch.nn.Parameter, r : torch.nn.Parameter,q : torch.nn.Parameter, mmd_kernel_num : float, mmd_kernel_mul : float, margin_max : float, margin_min : float, nsl : int, nr : int, nsg : int, nss : int):
    ...
    if loss_type.lower() == "triple_triplet_loss":
        return triple_triplet_loss.Triple_Triplet_Loss(ng=ng,nf=nf, nr=nr,nw=nw,margin=margin, random_margin=random_margin, alpha=alpha, beta=beta, p=p, r=r, mmd_kernel_num=mmd_kernel_num, mmd_kernel_mul=mmd_kernel_mul)
    if loss_type.lower() == "compact_triplet_mmd_alt":
    ...
    if loss_type.lower() == "compact_triplet_mmd":
        return compact_triplet_mmd.Compact_Triplet_MMD(ng=ng,nf=nf, nr=nr,nw=nw,margin=margin, random_margin=random_margin, alpha=alpha, beta=beta, p=p, r=r, mmd_kernel_num=mmd_kernel_num, mmd_kernel_mul=mmd_kernel_mul)
    if loss_type.lower() == "compact_quadruplet_mmd":
        return compact_quadruplet_mmd.Compact_Quadruplet_MMD(ng=ng,nf=nf, nr=nr,nw=nw,margin=margin, random_margin=random_margin, alpha=alpha, beta=beta, p=p, r=r, mmd_kernel_num=mmd_kernel_num, mmd_kernel_mul=mmd_kernel_mul)
    if loss_type.lower() == "compact_triplet_mmd_entropy":
        return compact_triplet_mmd_entropy.Compact_Triplet_MMD_Entropy(ng=ng,nf=nf, nr=nr,nw=nw,margin=margin, random_margin=random_margin, alpha=alpha, beta=beta, p=p, r=r, mmd_kernel_num=mmd_kernel_num, mmd_kernel_mul=mmd_kernel_mul)
    if loss_type.lower() == "compact_triplet_mmd_adapt":
        return compact_triplet_mmd_adapt.Compact_Triplet_MMD_Adapt(ng=ng,nf=nf, nr=nr,nw=nw,margin=margin, random_margin=random_margin, alpha=alpha, beta=beta, p=p, r=r, mmd_kernel_num=mmd_kernel_num, mmd_kernel_mul=mmd_kernel_mul)
