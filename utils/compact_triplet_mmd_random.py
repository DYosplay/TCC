...
class Compact_Triplet_MMD_Random(nn.Module):
    def __init__(self, ng : int, nf : int, nw : int, margin : float, alpha : float, beta : float, p : float, r : float, mmd_kernel_num : float, mmd_kernel_mul : float):
        ...
        """
        super(Compact_Triplet_MMD_Random, self).__init__()
        # Hyperparameters
        self.ng = ng
        self.nf = nf
        self.nw = nw
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            len_a = lens[i * step]
            len_p = (lens[i * step + 1 : i * step + 1 + self.ng])[new_ng]
            len_n = (lens[i * step + 1 + self.ng : i * step + 1 + self.ng + self.nf])[new_nf]
