...
class dataset(object):
    ...
    def __init__(self, sigDict, slnPath, slnLevel=0.4, taskSize=1, taskNumGen=5, taskNumNeg=5, numSynthesis=10, prefix="MCYT"):
        ...
        slnPath_G, slnPath_F = self.synthesisPressure(sigDict, slnPath_G, slnPath_F)
        print (">>>>> Done <<<<<")
    ...
    def synthesisPressure(self, sigDict, slnPath_G, slnPath_F):
        for key in sigDict.keys():
            for idx, path in enumerate(sigDict[key][True]):
                pressure = path[:,2]
                for j in range(self.numSynthesis):
                    synPath = slnPath_G[key][idx*self.numSynthesis+j]
                    synPressure = interpPressure(pressure, nPoints=synPath.shape[0]).astype("float32")
                    slnPath_G[key][idx*self.numSynthesis+j] = numpy.concatenate((synPath, synPressure[:,None]), axis=1)
                    synPath = slnPath_F[key][idx*self.numSynthesis+j]
                    synPressure = interpPressure(pressure, nPoints=synPath.shape[0]).astype("float32")
                    slnPath_F[key][idx*self.numSynthesis+j] = numpy.concatenate((synPath, synPressure[:,None]), axis=1)
        return slnPath_G, slnPath_F

    def synthesis(self, sigDict, slnPath, prefix, cache=True):
        if os.path.exists("./cache/%s_synFullPaths_%d_%.1f.pkl"%(prefix, self.numSynthesis, self.slnLevel)):
            PATH_G, PATH_F = pickle.load(open("./cache/%s_synFullPaths_%d_%.1f.pkl"%(prefix, self.numSynthesis, self.slnLevel), 'rb'), encoding='iso-8859-1') #, encoding='iso-8859-1'
        else:
            PATH_G = defaultdict(list)
            PATH_F = defaultdict(list)
            for key in sigDict.keys():
                for idx in range(len(sigDict[key][True])):
                    Ps = numpy.load(os.path.join(slnPath, "%d/Pmatrix_G%d_%d.npy"%(key, key, idx)))
                    Rs = numpy.load(os.path.join(slnPath, "%d/residual_G%d_%d.npy"%(key, key, idx)))
                    ...
                    for i in range(self.numSynthesis):
                        PATH_G[key].append(synthesis.synthesis(Ps, Rs, forgery=False, dtype="float32", padPenUp=True, const=self.slnLevel))
                        PATH_F[key].append(synthesis.synthesis(Ps, Rs, forgery=True, dtype="float32", padPenUp=True, const=self.slnLevel))
                # print ("Synthesizing user %d..."%(key))
                sys.stdout.write("Synthesizing user %d...\r"%(key))
                sys.stdout.flush()
            if cache:
                pickle.dump([PATH_G, PATH_F], open("./cache/%s_synFullPaths_%d_%.1f.pkl"%(prefix, self.numSynthesis, self.slnLevel), 'wb'))
