...
class DsTransformer(nn.Module):
    ...
    def _compact_quadruplet_mmd(self, data, lens):
        """ Loss de um batch
        ...
        for i in range(0, self.nw):
            ...
            dist_g = torch.zeros(self.ng, dtype=data.dtype, device=data.device)
            dist_n = torch.zeros(self.nf, dtype=data.dtype, device=data.device)
            dist_q = torch.zeros(self.nf//2, dtype=data.dtype, device=data.device)
            ...
            f_a = negatives[torch.argmax(dist_n[5:])] # falsificação profissional mais distante da ancora
            len_f_a = len_n[torch.argmax(dist_n[5:])]
            for j in range(1+self.ng+(self.nf//2),len(negatives)):
                dist_q[j] = self.sdtw(f_a[None, :int(len_f_a)], negatives[j:j+1, :int(len_n[j])])[0] / (len_a + len_n[j])
            ...
            lk_quadruplet = F.relu(dist_g.unsqueeze(1) + self.margin - dist_q.unsqueeze(0))
            ...
            # lk2 = F.relu(dist_g.unsqueeze(1) + self.margin - dist_n[5:].unsqueeze(0)) * (1-self.p)
            # lv = (torch.sum(lk1) + torch.sum(lk2)) / (lk1.data.nonzero(as_tuple=False).size(0) + lk2.data.nonzero(as_tuple=False).size(0) + 1)
            lv = (torch.sum(lk_skilled) + torch.sum(lk_random) + torch.sum(lk_quadruplet)) / (lk_skilled.data.nonzero(as_tuple=False).size(0) + lk_random.data.nonzero(as_tuple=False).size(0) + lk_quadruplet.data.nonzero(as_tuple=False).size(0) + 1)
    ...
    def start_train(self, n_epochs : int, batch_size : int, comparison_files : List[str], result_folder : str, triplet_loss_w : float = 0.5, fine_tuning : bool = False, dataset_scenario : str = "stylus"):
        ...
        elif self.loss_type == 'grl' or self.loss_type == 'triplet_loss' or self.loss_type == 'quadruplet_loss' or self.loss_type == 'hard_triplet_mmd' or self.loss_type == 'triplet_mmd' or self.loss_type == 'triplet_coral' or self.loss_type == 'norm_triplet_mmd' or self.loss_type == 'compact_triplet_loss' or self.loss_type == 'compact_triplet_mmd' or self.loss_type == 'compact_quadruplet_mmd':
            optimizer = optim.SGD(self.parameters(), lr=self.lr, momentum=0.9)
            lr_scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=self.decay) 
        ...
        for i in range(1, n_epochs+1):
            ...
            while epoch != []:
                ...
                if self.loss_type == 'grl':
                    ...
                    loss.backward()
                elif self.loss_type == 'compact_quadruplet_mmd':
                    loss = self._compact_quadruplet_mmd(outputs, length)
                    loss.backward()
                else:
        ...
        for i in range(1, n_epochs+1):
            ...
            while epoch[0] != [] and epoch[1] != []:
                ...
                if self.loss_type == 'hard_triplet_mmd':
                    ...
                    loss = self._compact_triplet_loss(outputs, length)
                    loss.backward()
                elif self.loss_type == 'compact_quadruplet_loss':
                    loss = self._compact_quadruplet_mmd(outputs, length)
                    loss.backward()
