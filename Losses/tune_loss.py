...
class Tune_Loss(nn.Module):
    ...
    def forward(self, data, lens):
        ...
        total_loss = 0
        dists_g = torch.zeros(self.nw*self.ng, dtype=data.dtype, device=data.device)
        dists_n = torch.zeros(self.nw*self.nf, dtype=data.dtype, device=data.device)
        ...
        dg = F.relu(dists_g - torch.min(dists_n))
        dg = (torch.sum(dg) / (dg.data.nonzero(as_tuple=False).size(0)+1)) 
