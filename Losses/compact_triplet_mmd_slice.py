...
class Compact_Triplet_MMD_Slice(nn.Module):
    def __init__(self, ng : int, nf : int, nw : int, margin : float, alpha : float, beta : float, p : float, r : float, nsl : float, mmd_kernel_num : float, mmd_kernel_mul : float):
        """_summary_
        ...
        """
        super(Compact_Triplet_MMD_Slice, self).__init__()
        # Hyperparameters
        self.ng = ng
        self.nf = nf
        self.nw = nw
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            for j in range(len(positives)):
                x_step = int(len_a//self.n)
                y_step = int(len_p[j]//self.n)
                for n in range(1, int(self.n)):
                    dist_g[j] += (self.sdtw(anchor[None, x_step*(n-1):x_step*n], positives[j:j+1, y_step*(n-1):y_step*n])[0] /(64*(x_step + y_step)))[0]
                
                n = self.n
                dist_g[j] += (self.sdtw(anchor[None, x_step*(n-1):int(len_a)], positives[j:j+1, y_step*(n-1):int(len_p[j])])[0] /(64* ((len_a - x_step*(n-1)) + (len_p[j] - y_step*(n-1)))))[0]
    
            for j in range(len(negatives)):
                x_step = int(len_a//self.n)
                y_step = int(len_n[j]//self.n)
                for n in range(1, int(self.n)):
                    dist_n[j] += (self.sdtw(anchor[None, x_step*(n-1):x_step*n], negatives[j:j+1, y_step*(n-1):y_step*n])[0] /(64*(x_step + y_step)))[0]
                
                n = self.n
                dist_n[j] += (self.sdtw(anchor[None, x_step*(n-1):int(len_a)], negatives[j:j+1, y_step*(n-1):int(len_n[j])])[0] /(64* ((len_a - x_step*(n-1)) + (len_n[j] - y_step*(n-1)))))[0]
