...
class Compact_Triplet_MMD_Adapt(nn.Module):
    def __init__(self, ng : int, nf : int, nr : int, nw : int, margin : float, random_margin : float, alpha : float, beta : float, p : float, r : float, mmd_kernel_num : float, mmd_kernel_mul : float):
        ...
        """
        super(Compact_Triplet_MMD_Adapt, self).__init__()
        # Hyperparameters
        self.ng = ng
        self.nf = nf
        self.nw = nw
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            lk_skilled_margin = F.relu(dist_n[:self.nf].unsqueeze(0) - dist_g.unsqueeze(1)) + self.margin
            lk_skilled = F.relu((dist_g.unsqueeze(1) - dist_n[:self.nf].unsqueeze(0)) + lk_skilled_margin)
            # lk_skilled = F.relu((dist_g.unsqueeze(1) - dist_n[:self.nf].unsqueeze(0)) + self.margin)


            lk_random_margin  = F.relu(dist_n[self.nf:].unsqueeze(0) - dist_g.unsqueeze(1)) + self.random_margin
            lk_random =  F.relu((dist_g.unsqueeze(1) - dist_n[self.nf:].unsqueeze(0)) + lk_random_margin)
