...
class Compact_Triplet_MMD(nn.Module):
    ...
    def forward(self, data, lens, w1, w2):
        ...
        for i in range(0, self.nw):
            ...
            if w1 > 0:
                for j in range(len(negatives[:self.nf//2])):
                    dist_n[j] = self.sdtw(anchor[None, :int(len_a)], negatives[j:j+1, :int(len_n[j])])[0] / (len_a + len_n[j])

            if w2 > 0:
                for j in range(len(negatives[self.nf//2:])):
                    dist_n[self.nf//2 + j] = self.sdtw(anchor[None, :int(len_a)], negatives[j:j+1, :int(len_n[j])])[0] / (len_a + len_n[j])

            lk_skilled = F.relu(dist_g.unsqueeze(1) + self.margin - dist_n[:self.nf//2].unsqueeze(0)) * w1
            lk_random = F.relu(dist_g.unsqueeze(1) + self.margin - dist_n[self.nf//2:].unsqueeze(0)) * w2
