...
class Clustering_Triplet_MMD(nn.Module):
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            all_gs = torch.zeros(((len(positives),(len(positives)))), dtype=data.dtype, device=data.device)
            all_sk = torch.zeros(((len(negatives)//2,(len(negatives)//2))), dtype=data.dtype, device=data.device)
            ...
            for j in range(len(all_gs)):
                for k in range(len(all_gs)):
                    # if j != k: # pode ser útil testar sem essa verificação já que sdtw(a,a) != 0
                    all_gs[j][k] = self.sdtw(positives[j:j+1, :int(len_p[j])], positives[k:k+1, :int(len_p[k])])[0] / (len_p[j] + len_p[k])

            for j in range(len(all_sk)):
                for k in range(len(all_sk)):
                    # if j != k: # pode ser útil testar sem essa verificação já que sdtw(a,a) != 0
                    all_sk[j][k] = self.sdtw(negatives[j:j+1, :int(len_n[j])], negatives[k:k+1, :int(len_n[k])])[0] / (len_n[j] + len_n[k])

            lk_skilled = F.relu(dist_g.unsqueeze(1) + self.margin - dist_n[:n_skilleds].unsqueeze(0))
            lk_random = F.relu(dist_g.unsqueeze(1) + self.margin - dist_n[n_skilleds:].unsqueeze(0))

            ca = (torch.sum(all_gs.reshape(-1)) + torch.sum(dist_g)) / (self.ng * self.ng + 1)
            cb = (torch.sum(all_sk.reshape(-1)) + torch.sum(dist_n[:n_skilleds])) / ((n_skilleds) * (n_skilleds) + 1)
            # cb = torch.mean(dist_n[:n_skilleds])
            intra_loss = torch.sum(dist_g - ca) + torch.sum(all_gs.reshape(-1) - ca)
