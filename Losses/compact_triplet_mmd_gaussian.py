...
class Compact_Triplet_MMD_gaussian(nn.Module):
    def __init__(self, ng : int, nf : int, nw : int, margin : float, alpha : float, beta : float, p : float, r : float, mmd_kernel_num : float, mmd_kernel_mul : float, mmin : float, mmax : float):
        """_summary_
        ...
        """
        super(Compact_Triplet_MMD_gaussian, self).__init__()
        # Hyperparameters
        self.ng = ng
        self.nf = nf
        self.nw = nw
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            for j in range(len(positives)):
                pst = positives[j:j+1, :int(len_p[j])]
                sigma = torch.rand(1) * (self.mmax - self.mmin) + self.mmin
                npst = torch.exp(-(pst**2)/(2*(sigma**2)).cuda()) / (torch.sqrt(2.0 * sigma * torch.pi)).cuda()
                
                dist_g[j] = self.sdtw(anchor[None, :int(len_a)], npst)[0] / (len_a + len_p[j])

            for j in range(len(negatives)):
                ngt = negatives[j:j+1, :int(len_n[j])]
                sigma = torch.rand(1) * (self.mmax - self.mmin) + self.mmin
                nngt = torch.exp(-(ngt**2)/(2*(sigma**2)).cuda()) / (torch.sqrt(2.0 * sigma * torch.pi)).cuda()

                dist_n[j] = self.sdtw(anchor[None, :int(len_a)], nngt)[0] / (len_a + len_n[j])
