...
class Double_Anchor_CTM(nn.Module):
    def __init__(self, ng : int, nf : int, nw : int, margin : float, alpha : float, beta : float, p : float, r : float, mmd_kernel_num : float, mmd_kernel_mul : float):
        ...
        """
        super(Double_Anchor_CTM, self).__init__()
        # Hyperparameters
        self.ng = ng
        self.nf = nf
        self.nw = nw
    ...
    def forward(self, data, lens):
        ...
        for i in range(0, self.nw):
            ...
            double_anchor_idx = torch.argmax(dist_g)
            double_anchor = positives[double_anchor_idx]
            len_da = len_p[double_anchor_idx]
            dist_dag = torch.zeros((len(positives)), dtype=data.dtype, device=data.device)
            dist_daf = torch.zeros((len(negatives)), dtype=data.dtype, device=data.device)
            dist_dag[double_anchor_idx] = dist_g[double_anchor_idx]
            for j in range(1,len(positives)):
                if j == double_anchor_idx: continue
                dist_dag[j] = self.sdtw(double_anchor[None, :int(len_da)], positives[j:j+1, :int(len_p[j])])[0] / (len_da + len_p[j])

            for j in range(len(negatives)):
                dist_daf[j] = self.sdtw(double_anchor[None, :int(len_da)], negatives[j:j+1, :int(len_n[j])])[0] / (len_da + len_n[j])

            
            lk_skilled_da = F.relu(dist_dag.unsqueeze(1) + self.margin - dist_daf[:5].unsqueeze(0))
            lk_random_da = F.relu(dist_dag.unsqueeze(1) + self.margin - dist_daf[5:].unsqueeze(0))
            ...
            ca2 = torch.mean(dist_dag)
            cb2 = torch.mean(dist_daf[:5])
            intra_loss2 = torch.sum(dist_dag - ca2)
            inter_loss2 = torch.sum(F.relu(self.beta - ((ca2 - cb2).norm(dim=0, p=2))))
            ...
            lv += (torch.sum(lk_skilled_da) + torch.sum(lk_random_da)) / (lk_skilled_da.data.nonzero(as_tuple=False).size(0) + lk_random_da.data.nonzero(as_tuple=False).size(0) + 1)


            user_loss = lv + intra_loss * self.p + inter_loss * self.r + intra_loss2 * self.p + inter_loss2 * self.r
