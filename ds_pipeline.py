    ...
    def start_train(self, comparison_files : List[str], result_folder : str):
        ...
        for i in range(1, self.hyperparameters['epochs']+1):
            ...
            # if True:
                ...
                from Losses import compact_triplet_mmd, syn_compact_triplet_mmd

                if self.hyperparameters['nss'] == 0:
                    self.loss_function = compact_triplet_mmd.Compact_Triplet_MMD(ng=self.hyperparameters['ng'],nf=self.hyperparameters['nf']*2,nw=self.hyperparameters['nw'],margin=1, alpha=self.hyperparameters['alpha'], beta=self.hyperparameters['beta'], p=self.hyperparameters['p'], r=self.hyperparameters['r'], mmd_kernel_num=self.hyperparameters['mmd_kernel_num'], mmd_kernel_mul=self.hyperparameters['mmd_kernel_mul'])
                else:
                    self.loss_function = syn_compact_triplet_mmd.Syn_Compact_Triplet_MMD(ng=self.hyperparameters['ng'], ns = self.hyperparameters['nf'],nr=self.hyperparameters['nr'], nsg=self.hyperparameters['nsg'],nss=self.hyperparameters['nss'],nw=self.hyperparameters['nw'],margin=1, alpha=self.hyperparameters['alpha'], beta=self.hyperparameters['beta'], p=self.hyperparameters['p'], r=self.hyperparameters['r'], mmd_kernel_num=self.hyperparameters['mmd_kernel_num'], mmd_kernel_mul=self.hyperparameters['mmd_kernel_mul'])
